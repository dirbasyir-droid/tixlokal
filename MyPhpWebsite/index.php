...
$sql_base = "SELECT c.*, (SELECT COUNT(*) FROM concert_seats s WHERE s.concert_id = c.id AND s.status = 'available') AS availability FROM concerts c WHERE c.artist LIKE '%$search_query%' OR c.venue LIKE '%$search_query%'";
switch ($sort_option) {
...
if (isset($_POST['resend_verification'])) {
    ...
    if ($res && mysqli_num_rows($res) === 1) {
        ...
        // Only resend if not verified (or column doesn't exist yet)
        if (isset($user['email_verified']) && (int)$user['email_verified'] === 1) {
            ...
            mysqli_query($conn, "UPDATE users
                SET verify_token='$token',
                    verify_expires=DATE_ADD(NOW(), INTERVAL 24 HOUR),
                    email_verified=0
                WHERE id=".(int)$user['id']
            );
            send_verification_email($user['email'], $user['name'], $token);
            $success = "Verification link sent. Please check your email (or use the demo link shown).";
            $action = 'login';
        }
    } else {
...
if (isset($_POST['login'])) {
    ...
    if ($res && mysqli_num_rows($res) == 1) {
        ...
        // Block login until email verified (if feature is enabled in DB)
        if (isset($user['email_verified']) && (int)$user['email_verified'] === 0) {
            ...
            // Auto-prepare resend button in UI
...
if (isset($_POST['register'])) {
    ...
    if ($check && mysqli_num_rows($check) > 0) {
        ...
        $ok = mysqli_query($conn, "INSERT INTO users (name, email, password, email_verified, verify_token, verify_expires)
            VALUES ('$name', '$email', '$hashed', 0, '$token', DATE_ADD(NOW(), INTERVAL 24 HOUR))"
        );

        if (!$ok) {
            // Fallback if columns not added yet (so the app still works)
            mysqli_query($conn, "INSERT INTO users (name, email, password) VALUES ('$name', '$email', '$hashed')");
            $success = "Registered! (Email verification columns not found yet — run the migration SQL.)";
            $action = 'login';
        } else {
            send_verification_email($email, $name, $token);
            $success = "Registered! Please verify your email before signing in.";
            $action = 'login';
        }
    }
}
...
.alert.warn{border-color: rgba(245,158,11,.35); background: rgba(245,158,11,.12); color: rgba(255,255,255,.95);}
    ...
    <div class="auth-wrap">
      ...
      <div class="auth-card">
        ...
        <?php if (!empty($_SESSION['pending_verify_email'])): ?>
          <div class="alert warn" style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;">
            <div><strong>Not verified?</strong> Click resend and check your inbox.</div>
            <form method="post" action="index.php?action=login" style="margin:0">
              <input type="hidden" name="email" value="<?php echo htmlspecialchars($_SESSION['pending_verify_email']); ?>">
              <button class="btn" type="submit" name="resend_verification" value="1" style="padding:8px 12px">Resend verification</button>
            </form>
          </div>
        <?php endif; ?>

        <?php if (!empty($_SESSION['last_verify_link'])): ?>
          <div class="alert" style="border-color: rgba(6,182,212,.35); background: rgba(6,182,212,.12);">
            <strong>Demo link:</strong>
            <a href="<?php echo htmlspecialchars($_SESSION['last_verify_link']); ?>" style="text-decoration:underline">Verify email now</a>
          </div>
        <?php endif; ?>
        ...
        <form method="post" action="index.php?action=login" style="margin-top:12px">
          ...
          <div class="form-row">
            <button class="btn btn-primary" type="submit" name="login" value="1" style="width:100%;justify-content:center">Sign in</button>
          </div>
    ...
    <div class="auth-wrap">
      ...
      <div class="auth-card">
        ...
        <form method="post" action="index.php?action=register" style="margin-top:12px">
          ...
          <div class="form-row">
            <button class="btn btn-primary" type="submit" style="width:100%;justify-content:center" name="register" value="1">Create account</button>
          </div>
...
<div class="container">
  <div class="topbar">
    <a class="brand" href="index.php">
      <img src="assets/tixlokal_logo_badge.png" class="brandLogo" alt="TixLokal Logo">
...
<div class="container">
  ...
  <div class="carousel" aria-label="Featured concerts">
    <?php
      $banner_res = mysqli_query($conn, "SELECT c.*, (SELECT COUNT(*) FROM concert_seats s WHERE s.concert_id = c.id AND s.status = 'available') AS availability FROM concerts c ORDER BY c.id DESC LIMIT 3");
      $count = 0;
